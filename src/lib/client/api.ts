  ...
  try {
    response = await fetch(path, {
      ...
      headers: {
        ...
        // generated by the browser, and naming the type here would send a body
...
export const api = {
  ...
  /**
   * Sends the files the server asked for, and only those.
   *
   * Batched because an export can hold hundreds of attachments and one
   * multipart body per file would be needlessly slow, while one body for all of
   * them would be needlessly large. Each file is named by its export-relative
   * reference, which is how the server matches it to the plan.
   */
  uploadMedia: async (
    jobId: string,
    files: readonly { reference: string; file: File }[],
    onProgress?: (sent: number, total: number) => void,
  ): Promise<{ stored: number; refused: number }> => {
    const BATCH = 10;
    let stored = 0;
    let refused = 0;
    ...
    for (let start = 0; start < files.length; start += BATCH) {
      ...
      for (const item of batch) {
        form.append("file", item.file, item.reference);
      }

      const result = await request<{ stored: number; refused: number }>(
        "/api/media/upload",
        { method: "POST", body: form },
      );
      stored += result.stored;
      refused += result.refused;
      onProgress?.(Math.min(files.length, start + batch.length), files.length);
    }
